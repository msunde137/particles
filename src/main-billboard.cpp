...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (action != GLFW_PRESS)
    {
        ...
        {
        case GLFW_KEY_0:
            theRenderer->lookTo(vec3(0, 0, -3), vec3(0, 0, 1), vec3(0, 1, 0)); return;
...
static void cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (state == GLFW_PRESS)
    {
        ...
        else
        {
            ...
            forward = vec3(hozrot * vec4(up, 0.0f));
...
int main(int argc, char** argv)
{
    ...
    theRenderer->lookTo(vec3(0, 0, 1), vec3(0, 0, -1), vec3(0, 1, 0));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // calculate frame of referece for particle
        vec4 fa = vec4(glm::cross(-theRenderer->viewDirection(), theRenderer->viewUp()), 0);
        vec4 fb = vec4(theRenderer->viewUp(), 0);
        vec4 fc = vec4(-theRenderer->viewDirection(),0);
        mat4 frame = glm::translate(mat4(1), vec3(0)) * mat4(fa, fb, fc, vec4(0, 0, 0, 1));
        // render particle
        theRenderer->begin(imageId, ALPHA);
        theRenderer->quad(frame, vec4(0, 1, .3, 1));
        theRenderer->end();
